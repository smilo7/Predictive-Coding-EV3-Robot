...
def run(N, dt, V, V_p, Sigma_p, Sigma_u, us_sensor, g_params, provided_measurements):
    """
    ...
    for i in range(0, N):
        ...
        if len(provided_measurements) != 0: 
            u[i] = provided_measurements[2][i]
        else:
            u[i] = us_sensor.distance_centimeters #take sensor reading
